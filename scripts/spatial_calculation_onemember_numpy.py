...
# Shift longitude from 0-360 to -180 to 180 for Cartopy
xlon = np.where(xlon0 > 180, xlon0 - 360, xlon0)
sort_idx = np.argsort(xlon)
xlon = xlon[sort_idx]
field = field0[:, :, sort_idx]
nlon = len(xlon)
...
# (Optionally, you can add gridlines with labels if needed)
lons, lats = np.meshgrid(xlon, ylat)
clevs = np.arange(-50,55,5)
cmap = plt.cm.RdYlBu_r
CS1 = ax.contourf(lons, lats, clim[:,:]-273.15, clevs, cmap=cmap,
                  transform=ccrs.PlateCarree(), extend='both')
cb = fig.colorbar(CS1, orientation="horizontal", pad=0.05, ax=ax)
cb.set_label('$^{\circ}C$', fontsize=10)
plotname = 'clim_%s_%s_%s_%s_%s_%i-%i' % (variable, model, member, domain, season, iyr, fyr)
ax.set_title(plotname, fontsize=12)
fig.tight_layout()
fig.savefig(plotsDir + plotname + '.png', format='png')
plt.show()
...
cmap = plt.cm.bwr
CS1 = ax.contourf(lons, lats, stdnotrend[:,:], clevs, cmap=cmap,
                  transform=ccrs.PlateCarree(), extend='both')
cb = fig.colorbar(CS1, orientation="horizontal", pad=0.05, ax=ax)
cb.set_label('$^{\circ}C$', fontsize=10)
...
CS1 = ax.contourf(lons, lats, trend*10, clevs, cmap=plt.cm.seismic,
                  transform=ccrs.PlateCarree(), extend='both')
cb = fig.colorbar(CS1, orientation="horizontal", pad=0.05, ax=ax)
