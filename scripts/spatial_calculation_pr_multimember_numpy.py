...
##
#Open
ref=sourceData+variable+'_Amon_IPSL-CM6A-LR_historical_r1i1p1f1_gr_185001-201412.nc'
fhref= Dataset(ref, mode = 'r')
fieldref0= fhref.variables[variable][:]
#units=fhref.variables['tas'].units
units='mm/day' # kg m-2 s-1 to mm/day
fieldref0=3600*24*fieldref0
xlon0 = fhref.variables['lon'][:]
##Shifting longitude from 0 to 360 to -180 to 180 for Cartopy
## Cartopy expects longitudes in the range [-180, 180]
def shift_longitude(data, lons):
    lons = np.asarray(lons)
    data = np.asarray(data)
    idx = lons >= 180
    lons_shifted = lons.copy()
    lons_shifted[idx] -= 360
    sort_idx = np.argsort(lons_shifted)
    lons_shifted = lons_shifted[sort_idx]
    data_shifted = data[..., sort_idx]
    return data_shifted, lons_shifted

fieldref, xlon = shift_longitude(fieldref0, xlon0)
nlon = len(xlon)
...
for k in np.arange(0, nbr, 1):
    fileName = variable + '_Amon_IPSL-CM6A-LR_historical_r' + str(k + 1) + 'i1p1f1_gr_185001-201412.nc'
    print(fileName)
    fh = Dataset(sourceData + fileName)
    field0 = fh.variables[variable][:]
    field0 = 3600 * 24 * field0
    lon0 = fh.variables['lon'][:]
    # Use the same shift_longitude function as above for Cartopy compatibility
    field, lon = shift_longitude(field0, lon0)
...
fig, axes = plt.subplots(nrows, ncols, figsize=(18, 16), subplot_kw={'projection': ccrs.PlateCarree()})
for k, ax in enumerate(axes.flatten()):
    ax.set_global()
    ax.coastlines()
    ax.add_feature(cfeature.BORDERS, linewidth=0.5)
    lons, lats = np.meshgrid(xlon, ylat)
    clevs = np.arange(0, 7.25, 0.25)
    cmap = plt.cm.BrBG
    CS1 = ax.contourf(lons, lats, clim[:, :, k], clevs, cmap=cmap, extend='both', transform=ccrs.PlateCarree())
    ax.annotate("%i" % (k + 1), xy=(0.02, 0.95), bbox={'facecolor': 'w', 'edgecolor': 'white', 'alpha': 0.08},
...
cb.set_label('%s'%(units), fontsize=8)
...
for k, ax in enumerate(axes.flatten()):
    ...
    cmap = plt.cm.rainbow
    CS1 = ax.contourf(lons, lats, std[:, :, k], clevs, cmap=cmap, extend='both', transform=ccrs.PlateCarree())
    ax.annotate("%i" % (k + 1), xy=(0.02, 0.95), bbox={'facecolor': 'w', 'edgecolor': 'white', 'alpha': 0.08},
                fontsize=8, xycoords='axes fraction')
plt.subplots_adjust(top=0.90, bottom=0.14, left=0.05, right=0.95, hspace=0.1, wspace=0.1)
#cax=fig.add_axes([left,bottom,width,height])
...
for k, ax in enumerate(axes.flatten()):
    ...
    clevs = np.arange(0, 2.1, 0.1)
    cmap = plt.cm.rainbow
    CS1 = ax.contourf(lons, lats, stdnotrend[:, :, k], clevs, cmap=cmap, extend='both', transform=ccrs.PlateCarree())
    ax.annotate("%i" % (k + 1), xy=(0.02, 0.95), bbox={'facecolor': 'w', 'edgecolor': 'white', 'alpha': 0.08},
                fontsize=8, xycoords='axes fraction')
plt.subplots_adjust(top=0.90, bottom=0.14, left=0.05, right=0.95, hspace=0.1, wspace=0.1)
#cax=fig.add_axes([left,bottom,width,height])
...
for k, ax in enumerate(axes.flatten()):
    ...
    cmap = plt.cm.BrBG
    CS1 = ax.contourf(lons, lats, trend[:, :, k] * 10, clevs, cmap=cmap, extend='both', transform=ccrs.PlateCarree())
    ax.annotate("%i" % (k + 1), xy=(0.02, 0.95), bbox={'facecolor': 'w', 'edgecolor': 'white', 'alpha': 0.08},
                fontsize=8, xycoords='axes fraction')
plt.subplots_adjust(top=0.90, bottom=0.14, left=0.05, right=0.95, hspace=0.1, wspace=0.1)
#cax=fig.add_axes([left,bottom,width,height])
cax=fig.add_axes([0.20,0.1,0.5,0.03])
cb = fig.colorbar(CS1,cax=cax,orientation="horizontal")
cb.set_label('%s/dec'%(units), fontsize=8)
