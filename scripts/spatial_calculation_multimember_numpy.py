...
##Shifting longitude from 0-360 to -180 to 180 using Cartopy-compatible approach
xlon = xlon0
fieldref = fieldref0

# If longitudes are from 0 to 360, convert to -180 to 180
if np.any(xlon > 180):
    xlon = ((xlon + 180) % 360) - 180
    # Sort longitudes and corresponding data
    sort_idx = np.argsort(xlon)
    xlon = xlon[sort_idx]
    fieldref = fieldref0[..., sort_idx]
...
for k, ax in enumerate(axes.flatten()):
    ...
    CS1 = ax.contourf(lons, lats, clim[:,:,k]-273.15, clevs,
                      cmap=cmap, transform=ccrs.PlateCarree(), extend='both')
    ax.coastlines()
    ax.add_feature(cfeature.BORDERS, linestyle=':')
    ax.annotate("%i" % (k+1), xy=(0.02, 0.95),
                bbox={'facecolor':'w', 'edgecolor':'white', 'alpha':0.08},
...
for k, ax in enumerate(axes.flatten()):
    ...
    cmap = plt.cm.rainbow
    CS1 = ax.contourf(lons, lats, std[:,:,k], clevs,
                      cmap=cmap, transform=ccrs.PlateCarree(), extend='both')
    ax.coastlines()
    ax.annotate("%i" % (k+1), xy=(0.02, 0.95),
                bbox={'facecolor':'w', 'edgecolor':'white', 'alpha':0.08},
...
for k, ax in enumerate(axes.flatten()):
    ...
    clevs = np.arange(-2,2.1,0.1)
    cmap = plt.cm.rainbow
    CS1 = ax.contourf(lons, lats, stdnotrend[:,:,k], clevs,
                      cmap=cmap, transform=ccrs.PlateCarree(), extend='both')
    ax.coastlines()
    ax.annotate("%i" % (k+1), xy=(0.02, 0.95),
                bbox={'facecolor':'w', 'edgecolor':'white', 'alpha':0.08},
...
for k, ax in enumerate(axes.flatten()):
    ...
    cmap = plt.cm.seismic
    CS1 = ax.contourf(lons, lats, trend[:,:,k]*10, clevs,
                      cmap=cmap, transform=ccrs.PlateCarree(), extend='both')
    ax.coastlines()
    ax.annotate("%i" % (k+1), xy=(0.02, 0.95),
                bbox={'facecolor':'w', 'edgecolor':'white', 'alpha':0.08},
